...
A:
- **Admin** - Acesso total, cada admin tem seu RA
- **Gestor** - Avalia, cria Nine Box, vê equipe (RA é o número que a pessoa já possui)
- **Colaborador** - Vê só próprio perfil (RA é o número que a pessoa já possui)
...
**Q: O que é RA?**
A: Registro Acadêmico do ENIAC - número único que cada pessoa já tem (entre 5 e 10 caracteres alfanuméricos)
...
npm run prisma:seed
```
Login: `admin@eniac.edu.br` / `admin123` (RA: use RA real)
